#!/bin/bash
set -eo pipefail
# usage: bash train-icelandic-model.sh $SRC $TRG $DATA $PROJECT $MODEL_NAME $RND $TODAY
# author: laurie
# trains a model with the hyperparameter settings for Is->En
...
PAR_MODEL="$MODELS_DIR/${SRC}${TRG}/${SRC}${TRG}-parallel-only/${SRC}${TRG}-parallel-only-${RND}-*/checkpoint_best.pt"
